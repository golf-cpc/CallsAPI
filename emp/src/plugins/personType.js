...
export const personType = (pid = 0, natCode = null) => {
  if (isOrs67(pid, natCode)) return 'บุคคลไม่ใช่สัญชาติไทย'
  if (isLpop(pid)) return 'บุคคลไม่มีสถานะทางทะเบียน'
  if (isArs(pid)) return 'บุคคลแรงงานต่างด้าว'
  if (isOrs(pid, natCode)) return 'บุคคลนี้สัญชาติไทย'
